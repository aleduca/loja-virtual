...
$(document).ready(function(){ 
  ...
  $('#wrap-categories .expanded').click(function() {
    if($("ul#category-menu").css("display") == 'none'){
      $('#wrap-categories .expanded').css("background", "url(/assets/images/cat-expanded-icon.png) no-repeat center center");
      $("ul#category-menu").show();
    }
    else{
      $('#wrap-categories .expanded').css("background", "url(/assets/images/cat-collapsed-icon.png) no-repeat center center"); 
  ...
  $("ul#category-menu li").click(
    function () {
      if( $(this).find("ul.subcategory").length > 0){
        ...
        if(status == "none"){
          $(this).find('a.extends').css("background", "#EEB900 url(/assets/images/submenu-arrow.png) no-repeat 210px center"); 
          $(this).find('a.extends').css("color", "#363031"); 
          $(this).find("ul.subcategory").css("display","block");
        }
        else{
          $(this).find('a.extends').css("background", "#2A2A2A url(/assets/images/cat-menu-arrow.png) no-repeat 210px center"); 
